...
if (len(LpImg)):
    ...
    cv2.imshow("Anh bien so phan nguong", binary)
    ...
    # Thực hiện phép biến đổi hình thái nâng cao
    ...
    for c in sort_contours(cont):
        ...
        if 1.5<=ratio<=3.5: # Chon cac contour dam bao ve ratio w/h
            if h/roi.shape[0]>=0.6: # Chon cac contour cao tu 60% bien so tro len bien 1 dong
    ...
    cv2.imshow("Bien so Tim Duoc", roi)
    cv2.waitKey(delay=30)

    # Viet bien so len anh
    cv2.putText(Ivehicle,fine_tune(plate_info),(50, 50), cv2.FONT_HERSHEY_PLAIN, 2, (0, 100, 255), lineType=cv2.LINE_AA)
